...
class AgregarValoracionUseCaseTest {
    ...
    void agregarValoracionHappyPath(){
        ...
        var events = (ValoracionAgregada)response.getDomainEvents().get(0);
        Assertions.assertEquals(3,events.getPuntuacion().value());
        Assertions.assertEquals("xx-xx",events.getIdValoracion().value());
        Assertions.assertEquals("id-user-1",events.getAutor().value().idUsuario().value());
        Assertions.assertEquals("Sebastian Cano",events.getAutor().value().nombre());
    }
